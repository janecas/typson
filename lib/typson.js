...
(function(definition) {
    ...
    if (typeof exports === "object") {
        module.exports = definition(require("underscore"), require("q"), null, null, require("fs"), require("path"), require("hasha"));
        // RequireJS
    } else if (typeof define === "function" && define.amd) {
        return define(["vendor/underscore", "vendor/q", "vendor/superagent", "vendor/typescriptServices", "fs",
            "path", "hasha"], definition);
    }
})(function(underscore, Q, request, TypeScript, fs, path, hasha) {
    if (underscore) {
        _ = underscore;
    }
    var api = {};
    ...
    function inlineContent(filepath, fileStr) {
        fileStr = fileStr.replace(/\{\{#\s*([^ ]+?)\s*#\}\}/g, function(whole, first) {
            try {
                return fs.readFileSync(path.join(path.dirname(filepath), first)).toString();
            } catch (ex) {
                return whole;
            }
        });
    ...
    function load(context, scripts) {
        ...
        var promises = _.map(scripts, function(locationOrScript) {
            ...
            if (context.files.indexOf(locationOrScript) === -1) {
                locationOrScript = normalize(locationOrScript);
                return loadScript(locationOrScript)
                    .then(function(script) {

                        script = convertInlineComments(script);
                        script = inlineContent(locationOrScript, script);

                        var scriptHash = hasha(script);

                        if (context.fileHashes.indexOf(scriptHash) !== -1) {
                            return;
                        }
                        ...
                        context.files.unshift(locationOrScript);
                        context.fileHashes.unshift(scriptHash);
                        // Parse the file
                        ...
                        var diagnostics = {
                            addDiagnostic: function(diagnostic) {
                                ...
                                console.warn(diagnostic.fileName() + "(" + (lineCol.line + 1) + "," +
                                    (lineCol.character + 1) + "): " +
                                    diagnostic.message());
                            }
                        };
    ...
    function fullPath(source, target) {
        var referencesDir = path.dirname(source);
        var result = path.resolve(referencesDir, target);
        return result;
    }

    /**
     * Remove the relative parts (../) inside a path
     * @param uri
     */
