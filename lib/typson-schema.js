...
(function(underscore, Q, _traverse, typson) {
    ...
    var annotedValidationKeywordPattern=/@[a-z\.]+\s+[\s\S]*?((?=@)|(?=$))/gi;
    ...
    api.schema = function(script, type, id) {
        return Q.promise(function(resolve) {
            api.definitions(script, "#/definitions").done(function(definitions) {
                ...
                var mainDefinition = definitions[type];
                if (!mainDefinition) {
                    type = type + '.' + type;
                    mainDefinition = definitions[type];
                }
                _.extend(schema, mainDefinition);
    ...
    function handlePropertyDeclaration(type, definition, definitions, refPath, modulePath) {
        _.each(type.members.members, function(variable) {
            ...
            // remove quotes from quoted property keys
            variable.id.actualText = variable.id.actualText.replace(/["']/g, '');
            ...
            //maps
                ...
                //if (JSON.stringify(variable).indexOf('Parameter') >= 0) {
                //    console.log(variable)
                //}
                ...
                if (members[0].returnTypeAnnotation) {
                    ...
                    if (!variableType) {
                        ...
                        handlePropertyDeclaration(variable.typeExpr.term.members.members[0].returnTypeAnnotation.term, property.additionalProperties, definitions, refPath, modulePath);
                        variableType = "any";
                    }
                }
                // Object (inline declaration)
                    ...
                    _.defaults(property, defaultProperties);
                    property.properties = {};
                    handlePropertyDeclaration(variable.typeExpr.term, property, definitions, refPath, modulePath);
                    variableType = "any";
                }
            }
            //other
            ...
            //enums
            var enumEntry = definitions.enums[modulePath + fullTypeName] || definitions.enums[fullTypeName];
            if (enumEntry) {
                propertyType.id = enumEntry.id;
                propertyType.enum = _.keys(enumEntry.enumeration);
                addEnumDescription(enumEntry.enumeration, property);
            }
            //other
    ...
    function getFullPathSegments(superType) {
        if (superType.actualText) {
            return [superType.actualText];
        }

        return getFullPathSegments(superType.operand1).concat(getFullPathSegments(superType.operand2));
    }

    function getFullPath(superType) {
        return getFullPathSegments(superType).join('.');
    }
    ...
    function mergeInheritedProperties(type, definition, definitions) {
        if (type.extendsList) {
            _.each(type.extendsList.members, function(superType) {
                ...
                var superDefinition = definitions.interfaces[fullPath];
    ...
    function addEnumDescription(enumeration, property) {
        if (enumeration && _.values(enumeration).join("")) {
            ...
            _.each(enumeration, function(comment, value) {
                table += "\n| `" + value + "`" + new Array(max + 1 - value.length).join(" ") + "|" +
                (comment ? " " + comment : "|");
            });
